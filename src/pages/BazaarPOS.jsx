...
import { Search, Plus, Minus, ShoppingBag, Gift, Tag, RefreshCw, ChevronDown, ChevronUp, X, Check, Layers, MapPin, Calendar, Download } from 'lucide-react';

const API  = 'https://siraj-backend.onrender.com';
const DAYS = ['Day 1', 'Day 2', 'Day 3'];

// ── Brand tokens (exact match to AdminDashboard) ──────────────────────────────
const DARK  = '#1E1023';
const ROSE  = '#BE185D';
const PINK  = '#F472B6';
const MID   = '#6B4A6E';
...
export default function BazaarPOS() {
  ...
  const startEvent = () => {
    ...
    // Generate a stable eventId from name + location
  ...
  const completeSale = async () => {
    ...
    if (!eventId) { alert('Please set up your event first.'); setShowEventSetup(true); return; }
    setSaving(true);
    setSaveMsg('');
    try {
  ...
  const exportCustomers = () => {
    const rows = [['Name', 'Phone', 'Event', 'Day', 'Date', 'Total EGP', 'Payment']];
    sales.forEach(s => {
      if (s.customerPhone) {
        rows.push([
          s.customerName,
    ...
    a.href = url;
    a.download = `siraj-bazaar-customers-${Date.now()}.csv`;
    a.click();
    ...
    <div style={{ fontFamily: 'Montserrat, sans-serif', color: DARK, paddingBottom: 80 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
        <span style={{ fontSize: 11, fontWeight: 700, color: MID, textTransform: 'uppercase', letterSpacing: '0.07em' }}>
          <Calendar style={{ width: 12, display: 'inline', marginRight: 4 }} />Recording:
        </span>
        {DAYS.map(d => (
          <button key={d} onClick={() => setCurrentDay(d)} style={{
            padding: '6px 16px', borderRadius: 20, border: '2px solid',
            borderColor: currentDay === d ? DAY_COLORS[d] : CREAM,
            background: currentDay === d ? DAY_COLORS[d] : '#fff',
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 14, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <button onClick={exportCustomers} style={{
              ...
              <Download style={{ width: 13 }} /> Export Customers CSV
            </button>
          </div>
          ...
          {loadingSales ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {daySales(filterDay).map(sale => {
                const dayCol     = DAY_COLORS[sale.bazaarDay] || ROSE;
                    ...
                    <div onClick={() => setExpandedSale(isExpanded ? null : sale._id)}
                      ...
                      <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                        <span style={{ fontSize: 10, padding: '3px 8px', borderRadius: 12, fontWeight: 700,
                          background: dayCol + '22', color: dayCol }}>{sale.bazaarDay}</span>
                        <div>
                    ...
                    {isExpanded && (
                      <div style={{ borderTop: `1px solid ${CREAM}`, padding: '14px 16px', background: PALE }}>
                        ...
                        <button onClick={() => deleteSale(sale._id)} style={{
                          marginTop: 12, padding: '5px 12px', background: '#fee2e2',
