...
    scents: [], // Will be used for the single 'Scent' field for relevant categories
    size: '',
...
    // --- NEW FIELDS ADDED HERE ---
    ...
    // Option-based specs (will be arrays in state, like 'scents')
    scentOptions: [],
    weightOptions: [],
    keyIngredients: [], // Using array for this
};
...
    // This handler is now primarily for the SINGLE 'Scent' field (Deodorant, Soap, etc.)
    const handleScentsChange = useCallback((e) => {
        const scentsArray = e.target.value.split(',').map(s => s.trim()).filter(Boolean);
        setFormData(prev => ({ ...prev, scents: scentsArray })); // We use 'scents' state for the single scent input now
    }, []);

    // --- NEW HANDLER for other comma-separated array fields ---
...
        // 2. Prepare productDetails object - UPDATED
        let productDetails = {
...
            coverageSpace: formData.coverageSpace,
            ...
            // --- ALL NEW FIELDS ADDED HERE (Text and Options) ---
            weight: formData.weight,
            skinType: formData.skinType,
            featureBenefit: formData.featureBenefit,
            color: formData.color,
            // Convert arrays back to comma-separated strings for backend
            scents: formData.scents.join(', '), // Your original 'scents' field used for single scent text
...
            productDetails.description_en = formData.description_en;
            ...
            // This ensures we don't send empty strings for fields that shouldn't exist
            switch(formData.category) {
                case 'Candles':
                case 'Pottery Collection':
                    // Keep candle fields, remove others
                    delete productDetails.weight; delete productDetails.skinType;
                    delete productDetails.featureBenefit; delete productDetails.color;
                    delete productDetails.scentOptions; delete productDetails.weightOptions;
                    delete productDetails.keyIngredients;
                    // Keep 'scents' as it might be used for naming, keep candle fields
                    break;
                case 'Deodorant':
                    // Keep Deodorant fields, remove others
                    delete productDetails.burnTime; delete productDetails.wickType;
                    delete productDetails.coverageSpace; delete productDetails.weight;
                    delete productDetails.featureBenefit; delete productDetails.color;
                    delete productDetails.scentOptions; delete productDetails.weightOptions;
                    // Keep 'scents', 'skinType', 'keyIngredients'
                    break;
                case 'Soap':
                     // Keep Soap fields, remove others
                    delete productDetails.burnTime; delete productDetails.wickType;
                    delete productDetails.coverageSpace; delete productDetails.skinType;
                    delete productDetails.color; delete productDetails.scentOptions;
                    delete productDetails.weightOptions;
                    // Keep 'scents', 'weight', 'featureBenefit', 'keyIngredients'
                    break;
                case 'Body Splash':
                    // Keep Body Splash fields, remove others
                    delete productDetails.burnTime; delete productDetails.wickType;
                    delete productDetails.coverageSpace; delete productDetails.weight;
                    delete productDetails.skinType; delete productDetails.featureBenefit;
                    delete productDetails.color; delete productDetails.scentOptions;
                    delete productDetails.weightOptions; delete productDetails.keyIngredients;
                    // Keep 'scents', 'size' (already in core)
                    break;
                case 'Shimmering Body Oil':
                    // Keep Oil fields, remove others
                    delete productDetails.burnTime; delete productDetails.wickType;
                    delete productDetails.coverageSpace; delete productDetails.skinType;
                    delete productDetails.featureBenefit; delete productDetails.scentOptions;
                    delete productDetails.weightOptions; delete productDetails.keyIngredients;
                     // Keep 'color', 'scents', 'weight'
                    break;
                case 'Massage Candles':
                     // Keep Massage Candle fields, remove others
                    delete productDetails.burnTime; delete productDetails.wickType;
                    delete productDetails.coverageSpace; delete productDetails.skinType;
                    delete productDetails.featureBenefit; delete productDetails.color;
                    delete productDetails.scentOptions; delete productDetails.weightOptions;
                ...
                case 'Car Diffusers':
                case 'Reed Diffusers':
                    // Keep Scent Options, remove others
                    delete productDetails.burnTime; delete productDetails.wickType;
                    delete productDetails.coverageSpace; delete productDetails.weight;
                    delete productDetails.skinType; delete productDetails.featureBenefit;
                    delete productDetails.color; delete productDetails.weightOptions;
                    delete productDetails.keyIngredients; delete productDetails.scents; // Remove single scent field
                    // Keep 'scentOptions'
                    break;
                case 'Wax Melts':
                     // Keep Weight/Scent Options, remove others
                    delete productDetails.burnTime; delete productDetails.wickType;
                    delete productDetails.coverageSpace; delete productDetails.skinType;
                    delete productDetails.featureBenefit; delete productDetails.color;
                    delete productDetails.keyIngredients; delete productDetails.scents; // Remove single scent field
                    // Keep 'scentOptions', 'weightOptions'
                    break;
                case 'Sets':
                case 'Fizzy Salts': // Assuming no extra fields yet
                     // Remove all optional spec fields
                    delete productDetails.burnTime; delete productDetails.wickType;
                    delete productDetails.coverageSpace; delete productDetails.weight;
                    delete productDetails.skinType; delete productDetails.featureBenefit;
                    delete productDetails.color; delete productDetails.scentOptions;
...
            }));
            // Remove all fields not applicable to bundles
            delete productDetails.scents; delete productDetails.size;
            delete productDetails.formattedDescription; delete productDetails.burnTime;
            delete productDetails.wickType; delete productDetails.coverageSpace;
            delete productDetails.weight; delete productDetails.skinType;
            delete productDetails.featureBenefit; delete productDetails.color;
            delete productDetails.scentOptions; delete productDetails.weightOptions;
            delete productDetails.keyIngredients;
...
        // 3. Create FormData (No changes here)
...
        data.append('productData', JSON.stringify(productDetails));
        console.log("Sending product data:", JSON.stringify(productDetails, null, 2)); // DEBUG LOG

        // 4. Determine API endpoint and method (No changes here)
        const url = isEditing ? `${API_BASE_URL}/api/products/${formData._id}` : `${API_BASE_URL}/api/products`;
        const method = isEditing ? 'PUT' : 'POST';

        // 5. Make API Call (No changes here)
        try {
...
        }));
        ...
        // --- NEW: Parse all new fields ---
        const keyIngredientsArray = (productToEdit.keyIngredients || '').split(',').map(s => s.trim()).filter(Boolean);
        const scentOptionsArray = (productToEdit.scentOptions || '').split(',').map(s => s.trim()).filter(Boolean);
        const weightOptionsArray = (productToEdit.weightOptions || '').split(',').map(s => s.trim()).filter(Boolean);
...
            scents: scentsArray, // Use 'scents' state for single scent field
            bundleItems: bundleItemsFormatted,
            selectedFiles: [], // Clear selected files
            imagePaths: productToEdit.imagePaths || [], // Keep existing image URLs

            // --- NEW: Set new fields in form state ---
            keyIngredients: keyIngredientsArray,
            scentOptions: scentOptionsArray,
            weightOptions: weightOptionsArray,
            // Text fields like weight, skinType are spread automatically
        });
        setIsEditing(true);
...
    const isBundle = formData.productType === 'Bundle';
    ...
    // --- NEW: Helper function to render conditional fields ---
    const renderCategorySpecificFields = () => {
        const { category } = formData;

        // Don't render anything if bundle or no category
...
                                </div>
                        {/* Add Scent field for Pottery/Candles if needed for naming */}
...
                                    <input type="text" name="weight" id="weight" value={formData.weight} onChange={handleChange} className="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-3 border" placeholder="e.g., 100g"/>
                                </div>
                         <div className="col-span-1">
...
                                </div>
                        <div className="col-span-1">
                                    <label htmlFor="weight" className="block text-sm font-medium text-gray-700 mb-1">Weight / Size</label>
                                    <input type="text" name="weight" id="weight" value={formData.weight} onChange={handleChange} className="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-3 border" placeholder="e.g., 50ml"/>
                                </div>
                    </>
                );
...
                                </div>
                        <div className="col-span-1">
                                    <label htmlFor="weight" className="block text-sm font-medium text-gray-700 mb-1">Weight</label>
                                    <input type="text" name="weight" id="weight" value={formData.weight} onChange={handleChange} className="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 p-3 border" placeholder="e.g., 150g"/>
                                </div>
                    </>
                );
            ...
            case 'Fizzy Salts':
                 return (
                    <div className="md:col-span-3 text-center text-gray-500 italic p-4">
                         ...
                         <br/>
                         Use the **Description** fields to detail what is included.
                    </div>
                );

            default:
                // Show the original candle fields as a fallback if category is unknown/old
                return (
                     <>
                        <div>
                                    <label htmlFor="burnTime" className="block text-sm font-medium text-gray-700 mb-1">Burn Time</label>
...
                    <div className="flex flex-wrap justify-between items-center gap-4 mb-6 border-b pb-4">
                    ...
                    {/* Global Message Area (No changes) */}
                    {message && (
...
                        {/* --- Product Type Toggle --- (No changes) */}
...
                            {/* Name Input (Conditional - no changes) */}
                            {isBundle ? (
                                <div className="col-span-1">
...
                             {/* Category - UPDATED with all new categories */}
                            <div className="col-span-1">
...
                                    <option value="">Select Category</option>
                                    ...
                                    {/* Removed old 'Freshener', 'Diffuser', 'Gift Set' */}
                                </select>
                            </div>
...
                            {/* Size (Only for Single Product - no changes) */}
                            {!isBundle && (
                                <div className="col-span-1">
...
                            )}
                            ...
                            {/* 'Scents' input moved to conditional section */}

                        </fieldset>

                        {/* --- Product Specifications Section (Now fully conditional) --- */}
                         <fieldset className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4 border-t pt-6">
                            ...
                            {/* Conditional fields rendered here */}
                            {renderCategorySpecificFields()}
                         </fieldset>


                        {/* --- Descriptions Section --- (No changes) */}
...
                         {/* --- Image Uploads Section --- (No changes) */}
...
                        {/* --- Bundle Items Section (Conditional - no changes) --- */}
                        {isBundle && (
...
                </div>
                ...
                {/* --- Loading Indicator, Product List, Order List, Order Modal --- */}
                {/* --- (NO CHANGES MADE TO THESE SECTIONS) --- */}
