                    ...
                    {(section.type === 'trust' || section.type === 'why_siraj') && (
                      <div className="md:col-span-2 border border-gray-200 rounded-lg p-3 bg-gray-50 space-y-2">
                        ...
                        {(section.items || []).map((item, itemIndex) => (
                          <div key={itemIndex} className="flex items-center gap-2 bg-white border rounded-lg p-2">
                            {item.imageUrl ? (
                              <img src={item.imageUrl} alt="" className="w-10 h-10 object-contain rounded border bg-gray-50 flex-shrink-0" />
                            ) : (
                              <span className="w-10 h-10 flex items-center justify-center text-xl border rounded bg-gray-50 flex-shrink-0">{item.icon || '✨'}</span>
                            )}
                            <div className="flex flex-col gap-1">
                              <input type="file" accept="image/*" onChange={e => uploadItemImage(index, itemIndex, e)} className="hidden" id={`trust-img-${index}-${itemIndex}`} />
                              <label htmlFor={`trust-img-${index}-${itemIndex}`} className="inline-flex items-center gap-1 px-2 py-1 bg-gray-100 border rounded text-[11px] cursor-pointer hover:bg-gray-200 whitespace-nowrap">
                                <Upload className="w-3 h-3" /> {itemUploadingKey === `${index}-${itemIndex}` ? 'Uploading...' : (item.imageUrl ? 'Replace icon' : 'Upload icon')}
                              </label>
                              {item.imageUrl && (
                                <button onClick={() => updateSectionItem(index, itemIndex, 'imageUrl', '')} className="text-[11px] text-red-500 hover:underline text-left">Remove, use emoji instead</button>
                              )}
                            </div>
                            {!item.imageUrl && (
                              <input value={item.icon} onChange={e => updateSectionItem(index, itemIndex, 'icon', e.target.value)}
                                className="w-14 px-2 py-2 border rounded-lg text-sm text-center" placeholder="🌿" />
                            )}
                        ...
                        <p className="text-[11px] text-gray-400">Upload a professional icon image (PNG/SVG works best), or skip that and just type an emoji instead.</p>
                      </div>
                    )}
